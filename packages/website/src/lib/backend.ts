import server$, {createServerAction$, ServerFunctionEvent } from "solid-start/server";
import {AUTH0_AUDIENCE, AUTH0_ID, AUTH0_SECRET, AUTH0_TOKEN_URL, ENDPOINT} from "~/lib/env";
...
const acquireToken: () => Promise<[any, number]> = async () => {
  ...
  if (!response.ok) {
    throw new Error("Could not acquire token: "+response.status+await response.text())
  }
 ...
 */
const backendFetchInternal = server$(async (opts: {route: string, init: RequestInit}, session: Session | null) => {
  const userEmail = session?.user?.email ?? "";
  console.log("Fetching from backend: "+opts.route+" with email "+userEmail)
  const newInit = {
    ...opts.init,
    headers: {
 ...
 */
export async function backendServersideFetch(route: string, init: RequestInit = {}, session: Session | null = null) {
...
export function createBackendFetchAction() {
  return createServerAction$(async (opts: {route: string, init: RequestInit}, event) => {
    return backendFetchInternal(opts, await getSession(event.request, authOpts))
  })
}